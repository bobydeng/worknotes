...
#ifdef SERVO
  pwm_gradient = SPINDLE_PWM_RANGE/(SERVO_MAX_ANGLE - SERVO_MIN_ANGLE);
#else
...
void spindle_set_speed(uint16_t pwm_value)
{
  ...
  uint16_t spindle_compute_pwm_value(float angle) // Mega2560 PWM register is 16-bit.
  {
	  uint16_t pwm_value;
	  // Calculate PWM register value based on max/min angle and programmed angle.
	  if (angle >= SERVO_MAX_ANGLE) {
	    sys.spindle_speed = SERVO_MAX_ANGLE;
	    pwm_value = SPINDLE_PWM_MAX_VALUE;
	  } else if (angle <= SERVO_MIN_ANGLE) {
		  sys.spindle_speed = SERVO_MIN_ANGLE;
		  pwm_value = SPINDLE_PWM_MIN_VALUE;
	  } else { 
	    sys.spindle_speed = angle;
	    pwm_value = floor((angle-SERVO_MIN_ANGLE)*pwm_gradient) + SPINDLE_PWM_MIN_VALUE;
	  }
	  
	  return(pwm_value);
  }
#elif defined(ENABLE_PIECEWISE_LINEAR_SPINDLE)
